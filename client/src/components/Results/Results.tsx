...
const Results = () => {
  ...
  const { data: scores, isLoading, isFetching } = useGameScore(gameId);

  const [totals, setTotals] = useState<Array<Player & { score: number }>>([]);
      ...
      const calculatedTotals = players.map((player) => ({
        ...
        score: scores
          ? Object.values(scores).reduce(
              (acc, roundScores) => acc + (roundScores[player.id] || 0),
              0,
            )
          : 0,
      }));
